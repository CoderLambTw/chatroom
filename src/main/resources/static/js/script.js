...
let stompClient;
let username;
let baseURL = "https://6bd1-150-117-238-205.jp.ngrok.io"; // 放入ngrok host
...
const connect = (event) => {
    ...
    if (username) {
        ...
        const socket = new SockJS(`${baseURL}/chat-example`);
...
const onMessageReceived = (payload) => {
    ...
    else flexBox.className = "d-flex justify-content-end flex-row-reverse mb-4";
    chatCard.appendChild(flexBox);

    const messageElement = document.createElement("div");
    messageElement.className = "msg_container_send";
    if (username !== message.sender) messageElement.classList.add("ml-3");
